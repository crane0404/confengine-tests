...
//    }
    ...
    @Given("User is on the HomePage and searched for {string}")
    public void userIsOnTheHomePageAndSearchedFor(String product) {
        LOGGER.info(System.out.printf("userOpensAmazonBaseURL - Persona:'%s', on platform: '%s'", AMAZON_ASSIGNMENT_TEST_CONTEXT.ME,
                Runner.platform));
        allDrivers.createDriverFor(AMAZON_ASSIGNMENT_TEST_CONTEXT.ME, Runner.platform, context);
        context.addTestState(AMAZON_ASSIGNMENT_TEST_CONTEXT.ME, AMAZON_ASSIGNMENT_TEST_CONTEXT.ME);
        new AmazonHomePageBL(AMAZON_ASSIGNMENT_TEST_CONTEXT.ME, Runner.platform).launchAmazonHomePage();
        new AmazonHomePageBL().userSearchesForProduct(product);
        new SearchResultsListsPageBL().searchedProductIsVisible(product);
    }

    @Then("User should be able to add product to the cart")
    public void userShouldBeAbleToAddProductToTheCart() {
        new SearchResultsListsPageBL().userSelectsFirstProduct();
        String productTitle = context.getTestStateAsString(AMAZON_ASSIGNMENT_TEST_CONTEXT.PRODUCT_TITLE);
        String productPrice = context.getTestStateAsString(AMAZON_ASSIGNMENT_TEST_CONTEXT.PRODUCT_PRICE);
        new ProductDescriptionPageBL().validateProductDetails(productTitle, productPrice);
        new ProductDescriptionPageBL().addProductToShoppingCart();
        new ProductDescriptionPageBL().openCartPage();
        new ShppingCartPageBL().verifyProductInCart(productTitle, productPrice);
    }
}
