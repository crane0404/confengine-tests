...
public class SearchResultsListsPageBL {
    ...
    public SearchResultsListsPageBL searchedProductIsVisible(String product) {
        ...
        boolean validateProductTitle = ProductListPageScreen.get().getFirstProductTitle().contains(product);
        softly.assertThat(validateProductTitle).as(String.format("Results Not visible for '%s'", product)).isTrue();
        String productTitle = ProductListPageScreen.get().getFirstProductTitle();
        context.addTestState(AMAZON_ASSIGNMENT_TEST_CONTEXT.PRODUCT_TITLE, productTitle);
        String productPrice = ProductListPageScreen.get().getFirstProductPrice();
        context.addTestState(AMAZON_ASSIGNMENT_TEST_CONTEXT.PRODUCT_PRICE, productPrice);
